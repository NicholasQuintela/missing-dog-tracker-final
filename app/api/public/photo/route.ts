...
export async function GET(request: NextRequest) {
  ...
  if (!path || !base) {
    return new Response("Invalid photo path", {
      status: 400,
  ...
  try {
    // Supabase is contacted only when this Vercel route executes on a CDN miss.
    const upstream = await fetch(
      `${base}/storage/v1/object/public/dog-photos/${path}`,
      { cache: "no-store" },
    )
    ...
    // Browser should revalidate; Vercel's CDN keeps the shared photo copy for 30 days.
