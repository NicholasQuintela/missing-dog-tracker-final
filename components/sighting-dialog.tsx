...
type Props = { open: boolean; onClose: () => void; defaultCenter: [number, number]; dogs: MissingDog[]; defaultDogId?: string | null; onCreated: (s: Sighting) => void }
...
export function SightingDialog({ open, onClose, defaultCenter, dogs, defaultDogId, onCreated }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (error) throw error
      onCreated(data as Sighting); onClose(); setTitle(""); setDescription(""); setContact(""); setPhotoFile(null); setPhotoPreview(null)
  ...
  return <Modal open={open} onClose={onClose} title="Report a sighting" description="Add a green map pin where you saw a dog that may be missing.">
    <form onSubmit={submit} className="flex flex-col gap-4">
      ...
      <div className="flex gap-3"><Button type="button" variant="outline" className="flex-1" onClick={onClose}>Cancel</Button><Button className="flex-1" disabled={submitting}>{submitting ? <><Loader2 className="size-4 animate-spin" />Saving…</> : "Post sighting"}</Button></div>
    </form>
  </Modal>
}
