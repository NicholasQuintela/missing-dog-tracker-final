...
export function ChatDialog({ open, onClose, conversationId, userId }: { open: boolean; onClose: () => void; conversationId: string | null; userId: string }) {
  ...
  return <Modal open={open} onClose={onClose} title="Private chat" description="Only the pet owner and the verified volunteer, sighting reporter, or finder can read this conversation.">
    <div className="flex h-[60vh] flex-col gap-3">
      <div className="flex-1 space-y-2 overflow-y-auto rounded-xl bg-muted/50 p-3">
        ...
        {messages.map(message => <div key={message.id} className={`flex items-end gap-1 ${message.sender_id === userId ? "justify-end" : "justify-start"}`}>
          {message.sender_id !== userId && <button type="button" onClick={() => setReportedMessageId(message.id)} className="rounded-full p-1.5 text-muted-foreground hover:bg-destructive/10 hover:text-destructive" aria-label="Report this message"><Flag className="size-3.5" /></button>}
          <div className={`max-w-[82%] rounded-2xl px-3 py-2 text-sm ${message.sender_id === userId ? "bg-primary text-primary-foreground" : "bg-card text-foreground"}`}><p className="whitespace-pre-wrap break-words">{message.body}</p><p className="mt-1 text-[10px] opacity-70">{new Date(message.created_at).toLocaleString()}</p></div>
        </div>)}
        <div ref={bottomRef} />
      </div>
      ...
      {reportedMessageId && <ReportAbuseDialog open={true} onClose={() => setReportedMessageId(null)} targetType="message" targetId={reportedMessageId} />}
    </div>
  </Modal>
}
