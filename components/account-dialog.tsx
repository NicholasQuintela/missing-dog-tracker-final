...
export function AccountDialog({ open, onClose, userEmail, userId, onSignOut }: { open: boolean; onClose: () => void; userEmail: string; userId: string; onSignOut: () => void }) {
  ...
  return <Modal open={open} onClose={onClose} title="My PawFinder account" description={userEmail}>
    <div className="flex flex-col gap-4">
      <div><h3 className="font-bold">My reports</h3><p className="text-sm text-muted-foreground">Dogs linked to your account.</p></div>
